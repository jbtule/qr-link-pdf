...
[<Fact>]
let ``does not run OCR on a page that already has real text`` () =
    let mutable calls = 0
    ...
    let countingEngine =
        { options with
            OcrEngine =
                Some(fun _ ->
                    calls <- calls + 1
                    [ { Text = "https://example.com/should-not-be-found"; Box = SKRectI(0, 0, 10, 10) } ]) }

    let pdf = textParagraph "Just an ordinary sentence with no links in it." (72f, 700f) 400f

    use input = new MemoryStream(pdf)
    let found = PdfQrLinker.scan countingEngine input
